...
export async function POST(request: Request) {
  ...
  if (session && session.user.gamemaster) {
    try {
      ...
      if (!killRecord)
        return new Response(
          JSON.stringify({ error: 'Kill record not found' }),
          { status: 404 }
        );
      ...
      return new Response(
        JSON.stringify({ message: 'Kill reverted successfully' }),
        { status: 200 }
      );
    } catch (error) {
      console.error('Error reverting kill:', error);
      return new Response(JSON.stringify({ error: 'Failed to revert kill' }), {
        status: 500,
      });
    }
  }
