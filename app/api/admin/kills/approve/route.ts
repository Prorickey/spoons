...
export async function POST(request: Request) {
  ...
  if(session && session.user.gamemaster) {
    try {
      ...
      return new Response(JSON.stringify({ message: "Kill approved successfully", kill: updatedKill }), { status: 200 });
    } catch (error) {
      console.error("Error approving kill:", error);
      return new Response(JSON.stringify({ error: "Failed to approve kill" }), { status: 500 });
    }
  }
