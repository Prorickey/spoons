  ...
  callbacks: {
    async signIn({ account, profile, user }) {
      ...
      if (
        ...
        if (!player) {
          let result = await prisma.gameConfiguration
            .findFirst({
              where: {
                key: 'sign_ups_open',
              },
    ...
    async session({ session, token }) {
      ...
      if (session && token.email) {
        ...
        if (player.currentTarget)
          session.user.currentTargetName = await prisma.user
            .findFirst({
              where: {
        ...
        if (player.killedBy)
          session.user.killedByName = await prisma.user
            .findFirst({
              where: {
