...
export async function POST(request: Request) {
  ...
  if (session) {
    ...
    if (ffa) {
      ...
      if (lastKill && lastKill.createdAt.getTime() + 1000 > Date.now())
        return new NextResponse(
    ...
    if (!ffa) {
      ...
      if (
        data.verificationName.trim().toLowerCase() !=
        `${nextTarget.firstName.trim().toLowerCase()} ${nextTarget.lastName.trim().toLowerCase()}`
      )
        return new NextResponse(
          'Verification name is incorrect. Please enter your next target to verify your kill. If this ' +
            'is a mistake, contact the spoonmaster to resolve it.',
          { status: 403 }
        );
